...
const Requests = props => {
  ...
  const setDataType = (endpoint, set) => {
    fetch(url + endpoint, { headers: { "cache-control": "no-store" } })
      .then(res => res.json())
  ...
  useEffect(() => {
    if (JSON.parse(localStorage.getItem("data")))
      setSelectChecks(JSON.parse(localStorage.getItem("data")));
  }, []);
  ...
  useEffect(
    () =>
      props.history.push(
        `s-${selectChecks.service}--b-${selectChecks.brand}--st-${selectChecks.style}`
      ),
    []
  );
  ...
  const changeHandler = e => {
    ...
    fetch(
      url +
        `parse_link?service_slug=${service}&brand_slug=${brand}&style_slug=${style}`,
  ...
  return (
    <div>
      <div className="request-child">{props.children}</div>
      <h1>Requests</h1>
      <select
        className="select"
        name="service"
        onChange={e => changeHandler(e)}
        value={selectChecks.service}
      >
        {serviceData.map((el, ind) => (
          <option key={ind} value={el.slug}>
            {el.label}
          </option>
        ))}
      </select>
      ...
      <select
        ...
        name="style"
        onChange={e => changeHandler(e)}
        value={selectChecks.style}
      >
        {styleData.map((el, ind) => (
          <option key={ind} value={el.slug}>
            {el.label}
          </option>
        ))}
